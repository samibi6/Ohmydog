...
const props = defineProps(['durees', 'tailles', 'services', 'etats']);

let confirmEntryDelete = ref(false);
let dureeDelete = ref(null);
let open = ref(false);

const form = usePrecognitionForm("post", route("duree.store"), {
    service_id: '',
    taille_id: '',
    etat_id: '',
    duree: '',
...
function confirmingEntryDeletion(duree) {
    dureeDelete.value = duree.nom;
    deleteForm.id = duree.id;
...
const deleteEntry = () => {
    confirmEntryDelete.value = false;
    deleteForm.delete(route('duree.delete'), {
        preserveScroll: true,
...
function trouverNomEtat(id) {
    // Recherche de l'objet avec l'id correspondant
    let objetTrouve = props.etats.find(objet => objet.id === id);
    ...
    <AppLayout title="Durée">
        ...
        <div v-if="open">
            <form @submit.prevent="submit" class="max-w-lg mx-auto">
                <h2 class="text-2xl text-center mb-4">Créer une durée</h2>
        ...
        <div v-if="durees.length === 0 && services.length != 0 && tailles.length != 0">
            <a class="bg-blue-500 p-5" :href="route('duree.generate')">Générer liste durées</a>
        </div>

        <div class="py-4 pb-10">
            <div v-for="duree in durees"
                class="border-2 w-fit mx-auto flex bg-white p-2 space-x-4 mb-4 items-center rounded-xl">
                <h2 class="text-2xl text-center mb-1">{{ trouverNomService(duree.service_id) }}</h2>
                <p class="text-center">{{ trouverNomTaille(duree.taille_id) }}</p>
                <p class="text-center">{{ trouverNomEtat(duree.etat_id) }}</p>
                <p class="text-center">{{ duree.duree }}</p>
                <a class="bg-blue-500 p-2 rounded-lg text-white font-medium hover:bg-blue-800"
                    :href="route('duree.edit', { duree })">Modifier</a>
                <a class="bg-red-500 p-2 rounded-lg text-white font-medium hover:bg-red-800"
                    @click="confirmingEntryDeletion(duree)">Supprimer</a>
            </div>
        </div>
        ...
        <DialogModal :show="confirmEntryDelete" @close="closeModal">
            ...
            <template #content>
                Êtes-vous sûr de vouloir supprimer la durée {{ dureeDelete }} ?
            </template>
